        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("Enforced by the macOS network filter. Continues even when this window is closed.")
                        .font(.system(size: 12))
            ...
            HStack(spacing: 22) {
                StatusKV(label: "Filter", value: "Running")
...
struct InstallStateView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 8) {
                Text("Install network protection")
                    .font(.system(size: 17, weight: .semibold))
                    .foregroundStyle(Theme.ink)
                Text("Blockme installs a macOS network filter that enforces your blocklist even when this window is closed. macOS may ask you to approve it once in Privacy & Security.")
            ...
            Text("Requires a local Xcode-signed build · macOS 13 or later")
        ...
        VStack(spacing: 14) {
            ...
            VStack(spacing: 6) {
                Text("Protection needs attention")
                    ...
                    .foregroundStyle(Theme.ink)
                Text("The network filter is not currently active. Your blocklist is preserved, but new requests are not being blocked until the filter comes back.")
                    .font(.system(size: 12.5))
                    .foregroundStyle(Theme.ink3)
                    .multilineTextAlignment(.center)
                    .frame(maxWidth: 320)
            ...
            HStack(spacing: 8) {
                Button("Reinstall filter", action: onReinstall)
                ...
                Button(action: onRetry) {
                    HStack(spacing: 8) {
                        ...
                        Text(model.isRetrying ? "Retrying…" : "Retry status")
                    }
                    .frame(height: 30)
                    .padding(.horizontal, 16)
                }
