...
public enum DomainNormalizationError: Error, LocalizedError {
    case empty
    case invalid(String)
...
public enum DomainNormalizer {
    public static func normalize(_ rawValue: String) throws -> String {
        ...
        if let url = URL(string: candidate), let host = url.host, !host.isEmpty {
            candidate = host
                  ...
                  let url = URL(string: "https://\(candidate)"), let host = url.host, !host.isEmpty {
            candidate = host
        }
    ...
    public static func hostnamesToBlock(for domain: String) -> [String] {
